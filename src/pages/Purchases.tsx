...
const Purchases = () => {
  ...
  const { data, isLoading, refetch, isFetching } = useGetPurchasesQuery(filters);
  ...
  const handleStatusChange = (val: string) => {
    setFilters(prev => ({ ...prev, status: val === "all" ? undefined : val as any, page: 1 }));
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Track procurement from suppliers and incoming stock.</p>
          </div>
        </div>
        {isAdmin && (
          <Button onClick={() => navigate("/purchases/add")} className="cursor-pointer shadow-lg shadow-primary/20">
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-lg border bg-card overflow-hidden shadow-sm">
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow>
                <TableHead className="py-4">Order Details</TableHead>
                  ...
                  <TableRow key={purchase.id} className="group hover:bg-muted/20 cursor-pointer transition-colors" onClick={() => navigate(`/purchases/${purchase.id}`)}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold">{purchase.vendor.name}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                       <Button variant="ghost" size="icon" onClick={() => navigate(`/purchases/${purchase.id}`)} className="h-8 w-8 rounded-full hover:bg-primary/10 hover:text-primary transition-colors">
          ...
          <Pagination className="w-auto mx-0">
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious 
                  href="#"
                  className={cn(data.pagination.page <= 1 && "pointer-events-none opacity-50")} 
                  onClick={(e) => {
                    e.preventDefault();
              ...
              <PaginationItem>
                <PaginationNext 
                  href="#"
                  className={cn(data.pagination.page >= data.pagination.totalPages && "pointer-events-none opacity-50")} 
                  onClick={(e) => {
                    e.preventDefault();
